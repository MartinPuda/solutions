...
(cond ((empty? int-list1) int-list2)
      ((empty? int-list2) int-list1)
      ((op (first int-list1)
           (first int-list2))
       (cons (first int-list1)
             (merge op
                    (rest int-list1)
                    int-list2)))
      (else (cons (first int-list2)
                  (merge op
                         int-list1
                         (rest int-list2))))))
...
(define (make-assoc [names : (Listof Symbol)]
                    [values : (Listof Number)]): (Listof Associate)
  (map2 (lambda (name value) (tuple name value))
        names values))
